...
Create another node called "current" and set it to point to this dummy node. We'll use "current" to keep track of where we 
are in the new merged list.
...
Once you're done with the merging, the "dummy" node will still be at the start. Update the head of the list to point to the node 
that comes immediately after this dummy node.
...
class LinkedList:
    ...
    def merge(self, LL2):
        ...
        while i < self.length and j < LL2.length:
            if self.head.value < LL2.head.value:
                temp = self.head
